...
const AccordionComponent = ({ accordionTitle, accordionContent }) => {
  return (
    <div className='accordion_item flex flex-col overflow-x-hidden rounded-xl bg-[#10191D]'>
      <div className='accordion_toggler flex cursor-pointer items-center justify-between px-10 py-6 mobile:px-5 mobile:py-3'>
        <span className='text-xl text-white mobile:text-base'>{accordionTitle}</span>
      ...
      <div className='accordion_content h-0 overflow-hidden px-10 py-0 mobile:px-5'>
        <pre className='whitespace-pre-wrap text-base leading-normal text-[#96B0BD] mobile:text-sm'>
          {accordionContent}
        </pre>
      </div>
    </div>
...
const FAQ = () => {
  ...
  return (
    <div className='relative py-40 mobile:py-20' ref={faqUseRef}>
      <div className='relative z-10 mx-auto max-w-4xl'>
        <h2
          className='text-center text-[150px] font-bold text-white mobile:text-7xl'
          id='faq_heading'
        >
          FAQ
        </h2>
...
During transactions, each party is protected by the Tri-Proof Smart Contracts. If there is ever an issue, one party needs only to escalate the issue to the DAO and they will be supported appropriately.`}
      ...
      <div
        ...
        className='absolute bottom-1/4 left-full h-[300px] w-[300px] rounded-full bg-[radial-gradient(50%_50%_at_50%_50%,_#DC4F13_0%,_#FF4C00_100%)] opacity-0 blur-[150px]'
      />
    </div>
  );
};
