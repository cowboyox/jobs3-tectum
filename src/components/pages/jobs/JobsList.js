...
const JobsList = () => {
  ...
  const [searchType, setSearchType] = useState(type == null ? 'normal' : type);
  ...
  useEffect(() => {
    if (searchType == 'ai') {
      ...
      api.get(`/api/v1/jobs/ai-search/${searchWord}`).then((data) => {
        // dataJobs = data.data.jobs
        setLoadingAIJobs(false);
        setAIJobs(data.data.jobs);
    ...
    <div className='search_page container'>
      ...
      >
        ...
        <Select
          ...
          onValueChange={(e) => setSearchType(e)}
        >
                  ...
                  >
                    ...
                    {searchType == 'ai' &&
                      aiJobs.map((job, index) => {
                        return <JobCard job={job} key={index} reason={aiReasons[index]} />;
                      })}
                  </InfiniteScroll>
                </div>
              </div>
