  ...
  return (
    <>
      {stepNumber < max && (
        ...
        >
          {isAuth === false ? 'Continue' : 'Save and continue'}
        </div>
      )}
      {stepNumber == max && (
        <button className={className} onClick={() => setIsResumePreview?.(true)} type='submit'>
          {isWaiting && <Spinner />}
          {isAuth === false ? 'End' : finalButtonName ? finalButtonName : 'Publish'}
        </button>
      )}
    </>
  );
