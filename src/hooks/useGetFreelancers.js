...
export const useGetFreelancers = (pageNum, itemsPerPage, searchText = '', filters = [], skills) => {
  return useQuery({
    cacheTime: Infinity,
      ...
      if (pageNum > 0 && itemsPerPage > 0) {
        try {
          ...
          const { data } = await api.get(
            `/api/v1/profile/get-all-freelancers?page=${pageNum}&limit=${itemsPerPage}&searchText=${searchText}&earned=${earned}&hoursBilled=${hoursBilled}&jobSuccess=${jobSuccess}&languages=${languages}&hourlyRate=${hourlyRate}&skills=${skills}`
          );
    ...
    queryKey: ['useGetFreelancers', pageNum, itemsPerPage, searchText, filters, skills.sort()],
    staleTime: Infinity,
  });
};
