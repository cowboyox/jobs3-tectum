...
const GigCard = ({ gig }) => {
  ...
  return (
    <div className="flex p-7 justify-between rounded-xl bg-[#10191d] mb-4 mobile:flex-col-reverse mobile:gap-3 mobile:p-3">
      ...
      <div className="flex gap-3 items-start mobile:justify-between relative">
        <Status status={gig.gigStatus} />
        <div className="relative">
          <button
            onClick={handleDropdownToggle}
            className="h-8 w-8 p-2 flex items-center justify-center rounded-full hover:bg-slate-700 transition cursor-pointer"
          >
            <BsThreeDots />
          </button>
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-[#1B272C] rounded-[6px] shadow-lg z-10">
              <Link href={`./edit-gig/${gig._id}`}>
              <button
                className="flex items-center gap-2 w-full px-4 py-2 text-left text-white hover:bg-gray-700 transition"
              >
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex w-full items-stretch gap-5 mobile:flex-col">
        ...
        <Link href="./post-gig" className="p-1 rounded-xl cursor-pointer text-base transition bg-[#DC4F13] hover:bg-white hover:text-black mobile:w-full mobile:text-center flex items-center w-40 text-center justify-center mobile:py-2">
          Post a New Gig
        </Link>
      </div>
