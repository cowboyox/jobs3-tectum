...
const Litepaper = () => {
		...
		if (!navigationData.prev) {
			return <button className='page_navigationBtn' style={{ width: '100%' }} onClick={() => getPageContent(navigationData.next)}>
				<div style={{ width: '70%', textAlign: 'left' }}>
					<p key={`preview_title`} className='text-black' style={{ fontSize: 14 }}>Next</p>
					<p key={`preview_content`} className='text-black' style={{ fontSize: 22 }}>{navigationData.next.title}</p>
				</div>
			...
			return <button className='page_navigationBtn' style={{ width: '100%' }} onClick={() => getPageContent(navigationData.prev)}>
				...
				<div style={{ width: '70%', textAlign: 'right' }}>
					<p key={`preview_title`} className='text-black' style={{ fontSize: 14 }}>Previous</p>
					<p key={`preview_content`} className='text-black' style={{ fontSize: 22 }}>{navigationData.prev.title}</p>
				</div>
			</button>
		} else {
			return <>
				...
				<button className='page_navigationBtn' onClick={() => getPageContent(navigationData.next)}>
					<div style={{ width: '70%', textAlign: 'left' }}>
						<p key={`next_title`} className='text-black' style={{ fontSize: 14 }}>Next</p>
						<p key={`next_content`} className='text-black' style={{ fontSize: 22 }}>{navigationData.next.title}</p>
					</div>
					<p className='text-black' style={{ fontSize: 22 }}>{`>`}</p>
				</button>
			</>
	...
	return (
		<Layout pageClass={`litepaper_page`} key={`litepaper_page`}>
							...
							if (item.type === "paragraph") {
								if (item.nodes[0].object === 'text' && item.nodes[0].leaves[0].marks.length > 0) {
									if (item.nodes[0].leaves[0].marks[0].type === 'bold')
										...
										return <div key={`content-text-1-${content_index}`} style={{ margin: '10px 0' }}>{item.nodes[0].leaves.map((node, index) => <span key={`content-text-1-${content_index}-span-${index}`} style={node.marks.length > 0 ? { fontStyle: node.marks[0].type } : null}>{node.text}</span>)}</div>
									}
									...
									return <>{
										item.nodes.map((node) => {
											...
											else if (node.object === 'inline') return <div key={`page_content_link_item_${node.nodes[0].leaves[0].text}`}><a href={node.nodes[0].leaves[0].text} style={{color: "orange"}}>{node.nodes[0].leaves[0].text}</a></div>
										})
									}</>
								}
			...
			<div key={`litepaper-mask`} id='mask' className='mask'><div>Loading...</div></div>
		</Layout>
	)
};
