...
async function getData(slug) {
	try {
		const res = await fetch(
			`https://main.jobs3.io/wp-json/wp/v2/jobs?_embed&slug=${slug}`
		);
...
const job = async ({ params }) => {
	...
	return (
		<Layout pageClass={"sl_job_page"}>
			<div className="blank_space" style={{ height: "20px" }}></div>
						...
						<div className="job_info">
							...
							<div className="sl_job_details">
								...
								<div className="sl_job_name">
									...
									<h2>
										{fixHTMLEntities(job[0].title.rendered)}
									</h2>
								</div>
							...
							<div className="job_count_details">
								<span>
									{timeSincePublication(
										job[0].jobsearch_field_job_publish_date
									)}
								</span>
